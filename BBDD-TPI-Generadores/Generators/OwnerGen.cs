                ...
                for (int i = 0; i < num; i++)
                {
                    ...
                    if (personRowsAffected > 0)
                    {
                        ...
                        if (userRowsAffected > 0)
                        {
                            ...
                            var insertOwnerQuery = $"INSERT INTO owners (id) VALUES (@id)";
                            int ownerRowsAffected = connection.Execute(insertOwnerQuery, new { id = userId });

                            if (ownerRowsAffected > 0)
                            {
                                Console.WriteLine($"{ownerRowsAffected} owner rows inserted successfully.");
                            }
                            else
                            {
                                Console.WriteLine("Owner insertion failed or no rows were inserted.");
                            }
                        }
                        else
                        {
