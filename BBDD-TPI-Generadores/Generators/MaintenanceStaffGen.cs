...
{
    public static class MaintenanceStaffGen
    {
                ...
                for (int i = 0; i < num; i++)
                {
                    ...
                    if (personRowsAffected > 0)
                    {
                        ...
                        if (employeeRowsAffected > 0)
                        {
                            ...
                            var insertMaintenanceStaffQuery = $"INSERT INTO maintenance_staff (id) VALUES (@id)";
                            int maintenanceStaffRowsAffected = connection.Execute(insertMaintenanceStaffQuery, new { id = employeeId });

                            if (maintenanceStaffRowsAffected > 0)
                            {
                                Console.WriteLine($"{maintenanceStaffRowsAffected} maintenance staff rows inserted successfully.");
                            }
                            else
                            {
                                Console.WriteLine("Maintenance staff insertion failed or no rows were inserted.");
                            }
                        }
                        else
                        {
